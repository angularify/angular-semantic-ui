...
return {
  ...
  link: function(scope, element, attrs, DropDownController) {
     ...
     */
    if (scope.disabled === 'disabled' || scope.disabled === 'true' || scope.disabled === true) {
      scope.dropdown_class += ' disabled';
    }

    if (scope.disable === 'disabled' || scope.isDisabled === 'true' || scope.isDisabled === true) {
      scope.dropdown_class += ' disabled';
    }

    /*
     * Watch for disabled changing
     */
    scope.$watch('disable', function(value) {
      if (scope.disable !== undefined) {
        if (value === 'disabled' || value === 'true' || value === true) {
          scope.dropdown_class += ' disabled';
        } else {
          scope.dropdown_class = scope.dropdown_class.replace(' disabled', '');
        }
      }
    });
